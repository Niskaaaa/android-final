...
public class MyDatabase {
    ...
    public ArrayList<Data> getarray(){            //获取listview中要显示的数据
        ...
        Cursor cursor = mydatabase.rawQuery("select ids,title,times,imp,type from mybook",null);
        cursor.moveToFirst();
        while(!cursor.isAfterLast()){
            ...
            String imp = cursor.getString(cursor.getColumnIndex("imp"));
            Data data = new Data(id,title,times,type,imp);
            arr.add(data);
            cursor.moveToNext();
        }
        mydatabase.close();
    ...
    public Data getTiandCon(int id){           //获取要修改数据（就是当选择listview子项想要修改数据时，获取数据显示在新建页面）
        mydatabase = myOpenHelper.getWritableDatabase();
        Cursor cursor=mydatabase.rawQuery("select title,content,type,imp from mybook where ids='"+id+"'" , null);
        cursor.moveToFirst();
        String title=cursor.getString(cursor.getColumnIndex("title"));
        String content=cursor.getString(cursor.getColumnIndex("content"));
        String type = cursor.getString(cursor.getColumnIndex("type"));
        String imp = cursor.getString(cursor.getColumnIndex("imp"));
        Data data=new Data(title,content,type,imp);
        mydatabase.close();
        return data;
    }
    ...
    public void toInsert(Data data){           //在表中插入新建的便签的数据
        mydatabase = myOpenHelper.getWritableDatabase();
        mydatabase.execSQL("insert into mybook(title,content,times,type,imp)values('"
                + data.getTitle()+"','"
                +data.getContent()+"','"
                +data.getTimes()+"','"
                +data.getType()+"','"
